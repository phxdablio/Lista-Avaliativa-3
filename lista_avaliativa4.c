...
void ordenar(Produto produtos[], int numProdutos){
    int i, j;
    char aux[30];
    for(i=0;i<numProdutos;i++){
...
void listarFabricantes(Fabricante fabricantes[], int numFabricantes){
    int i;
    ordenar(fabricantes, numFabricantes);
    for(i=0;i<numFabricantes;i++){
        printf("%s\n", fabricantes[i].marca);
    }
}

void ordenarf (Fabricante fabricantes[], int numFabricantes){
    int x,y;
    char auxf[30];
    for(x=0;x<numFabricantes;x++){
        for(y=x+1;y<numFabricantes;y++){
            if(strcmp(fabricantes[x].marca, fabricantes[y].marca)>0){
                strcpy(auxf, fabricantes[x].marca);
                strcpy(fabricantes[x].marca, fabricantes[y].marca);
                strcpy(fabricantes[y].marca, auxf);
            }
        }
    }
}
...
int main()
{
    ...
    Ordenar ordenar[MAX_PRODUTOS];
    //Ordenar ordenarf[MAX_FABRICANTES];
    int numFabricantes = 0;
    int numProdutos = 0;
    int i=0, j;
...
if(strlen(fabricantes[i].UF)< 2 || strlen(fabricantes[i].UF)>2 )
    ...
    listarFabricantes(fabricantes, numFabricantes);
    char outro;
    char opcao;
 do{
